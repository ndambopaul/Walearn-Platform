...
class StudentCourse(AbstractBaseModel):
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name="studentcourses")
...
class Attendance(AbstractBaseModel):
    ...
    marked_as = models.CharField(max_length=255, choices=ATTENDANCE_CHOICES, default="Pending")
...
class AssigmentSubmission(AbstractBaseModel):
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name="assigmentsubmissions")
    ...
    status = models.CharField(max_length=255, choices=ASSIGNMENT_STATUS_CHOICES, default="Pending")
    active = models.BooleanField(default=True)
